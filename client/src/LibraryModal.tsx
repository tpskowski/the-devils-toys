...
export function LibraryModal({
  ...
  const [mentionPreview, setMentionPreview] = useState<MediaAsset>();
  ...
  useEffect(() => {
    const close = (event: KeyboardEvent) => event.key === "Escape" && !editingId && requestClose();
    addEventListener("keydown", close);
    return () => removeEventListener("keydown", close);
  }, [editingId, onClose, tab]);
  ...
  function openWikiMention(mention: WikiMentionTarget) {
    if (mention.kind === "asset") {
      const asset = library.find((item) => item.id === Number(mention.target));
      if (asset) {
        requestTab("assets");
        setMentionPreview(asset);
        return;
      }
    }
  ...
  useEffect(() => {
    ...
    const asset = library.find((item) => item.id === assetToOpen);
    if (!asset) return;
    requestTab("assets");
    setMentionPreview(asset);
  }, [assetToOpen, library]);
  ...
  return (
    ...
    >
      <section
        className={`modal library-modal${wikiEnabled ? " library-modal-has-wiki" : ""}`}
        role="dialog"
        aria-modal="true"
      ...
      >
        <header>
          ...
          <button onClick={requestClose} aria-label="Close Library">
            <X />
          </button>
        </header>
        {wikiEnabled && (
          <nav className="library-tabs" aria-label="Library contents" role="tablist">
            ...
            <button
              ...
              aria-controls="library-wiki-panel"
        ...
        {tab === "assets" ? (
          <div className="library-assets-tab" id="library-assets-panel" role="tabpanel">
            <div
            ...
            <div className="library-assets-status">
              {error && (
            ...
            {mentionPreview && (
              <section className="reference-preview" aria-label={`Preview ${mediaLabel(mentionPreview)}`}>
                <MediaContent asset={mentionPreview} />
                <p>{mediaLabel(mentionPreview)}</p>
              </section>
            )}
                ...
                <div className="library-empty">
                  ...
                  <p>
                    {library.length
                      ...
                      : "Upload a Map, Scene, or Reference to get started."}
                  </p>
                </div>
              )}
              {items.map((item) => {
          ...
          <div id="library-wiki-panel" role="tabpanel">
            <WikiWorkspace
              roomId={roomId}
              isGm
