...
- **A reviewed result has three homes, and a system declares which of them it offers.** A slot is `stowInto` on a `roll-table` or a `listKey` on a `grant` or `packet`; the description box is `describeInto` or the step's own `joinInto`; the sheet's own `entries` panel is `entryInto`, which is the only way anything in a creation declaration reaches an `entries` field and is checked at install to name one. `creationEntryFrom` splits the book's line into a name and what it does at the first colon, taking the emphasis a book bolds a talent's name with off along with it — so `**Hot-Wiring:** You cross the right wires` becomes a talent called Hot-Wiring. The parse lives in `shared/` because the screen shows the title before the player presses the button and the server writes it when they do, and those have to be the same title. A step declaring `entryInto` offers its results for review even where it names no list at all: a talent is not gear, and there is no slot for it to have been going into.
- **`firstOf` chooses at random; `choose` hands that to the player.** Which of Monolith's three first-name tables a character's name comes off is a decision about the character rather than a die roll, and a player left to the coin rolls until it agrees with them. The chosen table is still rolled on the server and still checked against what the step declared, a roll naming no choice still chooses at random, and only one of a step's rolls may declare it, because one roll of a step carries one choice.
